...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const [googleSheetId, setGoogleSheetId] = useLocalStorage<string>('googleSheetId', '');
    const [googleApiKey, setGoogleApiKey] = useLocalStorage<string>('googleApiKey', '');
    const [isSyncing, setIsSyncing] = useState(false);

    // Chargement initial des identifiants depuis config.json
    useEffect(() => {
      fetch('/config.json')
        .then(response => response.json())
        .then(config => {
          if (config.googleSheetId) setGoogleSheetId(config.googleSheetId);
          if (config.googleApiKey) setGoogleApiKey(config.googleApiKey);
        })
        .catch(() => {
          addToast("Le fichier config.json n'est pas configuré correctement", 'warning');
        });
    }, [setGoogleSheetId, setGoogleApiKey, addToast]);
